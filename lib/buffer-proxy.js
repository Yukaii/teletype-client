...
class PendingEdit {
  constructor(textUpdates, markerUpdates) {
    this.textUpdates = textUpdates;
    this.markerUpdates = markerUpdates
  }
}
...
class BufferProxy {
  ...
  onDidChangeBuffer (textUpdates) {
    this.localChangesQueue.push(textUpdates);
    this._step()
  }

  applyLocalOperation ({ oldStart, oldEnd, newText }) {
    const operations = this.document.setTextInRange(oldStart, oldEnd, newText)
    this.broadcastUpdate(operations)
    this.emitter.emit('did-update-text', {remote: false})
  }
  ...
  integrateOperations (operations) {
    this.remoteOperationsQueue.push(operations)
    this._step()
  }
  ...
  _step() {
    ...
    // applied local changes first
    while (this.localChangesQueue.length > 0) {
      const operations = this.localChangesQueue.shift()
    ...
    // now deal with remote updates
    if (this.remoteOperationsQueue.length > 0) {
      const operations = this.remoteOperationsQueue.shift()
  ...
  applyRemoteOperations (operations) {
    ...
    this.pendingEdit = new PendingEdit(textUpdates, markerUpdates);

    if (this.delegate) {
      const fnReturn = this.delegate.updateText(textUpdates)
      if (fnReturn && typeof fnReturn.then === 'function') {
        fnReturn.then((applied) => {
          if (applied) {
            this.applyRemoteUpdatesOk()
          } else {
            this.pendingEdit = null
            this._step()
          }
  ...
  applyRemoteUpdatesOk () {
    const { textUpdates, markerUpdates } = this.pendingEdit
    this.pendingEdit = null

    // clears changes trigger with internal edits
